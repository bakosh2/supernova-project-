    ...
    var body: some View {
        NavigationStack {
            GeometryReader { geo in
                ZStack {
                    SpaceBackgroundView()
                    WelcomeExtraStarsView(twinkle: twinkle)
                    VStack {
                        Spacer()
                        PlanetSurfaceView()
                    }
                    ...
                    ZStack {
                        ...
                        Image("supernova_astronaut")
                            .resizable()
                            .scaledToFit()
                            .rotationEffect(.degrees(tiltLeft ? -4 : 4))
                            .shadow(color: Color("tiffany").opacity(0.35), radius: 15)
                            .offset(y: floatUp ? -14 : 10)
                    }
                    .frame(width: min(350, geo.size.width * 0.34))
...
private struct WelcomeExtraStarsView: View {
    let twinkle: Bool
    private let glowStars: [(CGFloat, CGFloat, CGFloat)] = [
        (0.09, 0.19, 34), (0.18, 0.22, 16), (0.13, 0.46, 18),
        (0.50, 0.50, 22), (0.94, 0.06, 24), (0.84, 0.29, 26),
        (0.68, 0.43, 18), (0.41, 0.69, 16), (0.90, 0.55, 20)
    ]
    private let sparkles: [(CGFloat, CGFloat, CGFloat)] = [
        (0.21, 0.17, 7), (0.40, 0.31, 6), (0.55, 0.20, 8), (0.63, 0.05, 6),
        (0.05, 0.36, 7), (0.15, 0.64, 6), (0.53, 0.55, 7), (0.78, 0.49, 8),
        (0.92, 0.40, 6), (0.83, 0.65, 7), (0.60, 0.73, 6), (0.35, 0.60, 7),
        (0.95, 0.75, 6), (0.72, 0.60, 7)
    ]

    var body: some View {
        GeometryReader { geo in
            ZStack {
                ForEach(glowStars.indices, id: \.self) { index in
                    let star = glowStars[index]
                    Image("supernova_star")
                        .resizable()
                        .scaledToFit()
                        .frame(width: star.2, height: star.2)
                        .shadow(color: .yellow.opacity(twinkle ? 0.9 : 0.4), radius: twinkle ? 8 : 3)
                ...
                ForEach(sparkles.indices, id: \.self) { index in
                    ...
                    Image("supernova_star_sparkle")
                        .resizable()
                        .scaledToFit()
                        .frame(width: sparkle.2, height: sparkle.2)
                        .opacity(twinkle ? 0.95 : 0.55)
                        .position(x: geo.size.width * sparkle.0, y: geo.size.height * sparkle.1)
                }
            }
        }
